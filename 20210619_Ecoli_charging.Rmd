...
setwd("~/4SR/data/20210619_Ecoli_charging_collaboration_round2/source/")
...
my_comparisons <- list( c("WT", "trmA-KO"),
                        c("WT", "truB-KO"),
                        c("truB-KO", "trmA-KO"))
#Reorder WT and KO so they look nice on the graph
temp$treatment <- factor(temp$treatment, levels=c("WT","trmA-KO", "truB-KO"))
...
temp$treatment <- recode(temp$treatment, "WT"="WT", "trmA-KO"="trmA", "truB-KO"="truB")
...
temp <- temp %>%
  ...
  mutate(charging_ratio_trmA = trmA / WT,
         charging_ratio_truB = truB / WT)

ggplot(filter(temp,
              AA!="Und"),
       aes(x=charging_ratio_truB)) +
  geom_density(color="red") +
  geom_density(aes(x=charging_ratio_trmA), color="blue")
...
plot1 <- ggplot(filter(temp,
  ...
  geom_point(aes(y=rpm / WT_mean_rpm)) +
  geom_hline(yintercept = 1, linetype=2, alpha=0.3) +
  facet_grid(rows=vars(treatment)) +
  ylab("rpm / mean of WT rpm") +
  theme(axis.text.x = element_text(angle=90, vjust=0.5),
        axis.title.x = element_blank(),
...
temp <- temp %>%
  ...
  mutate(trmA_WT_ratio = trmA / WT,
         truB_WT_ratio = truB / WT) %>%
  select(-WT, -truB, -trmA) %>%
  pivot_longer(c(-gene, -AA, -anticodon, -number), names_to=c("treatment"), values_to = c("ratio_rpm")) %>%
  filter(AA != "Und")
temp$treatment <- recode(temp$treatment, "trmA_WT_ratio" = "trmA-KO", "truB_WT_ratio" = "truB-KO")

plot1 <- ggplot(filter(temp, AA !="Und"),
       aes(x=treatment, y=ratio_rpm)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width=0.05) +
  geom_text_repel(data=filter(temp, ratio_rpm>1.25 | ratio_rpm <0.65),
                  aes(label=paste0(AA, anticodon, number)) ) +
  ylab("Abundance ratio: KO / WT") +
  xlab("Strain")
...
temp <- temp %>%
  ...
  mutate(delta_trmA = trmA - WT,
         delta_truB = truB - WT) %>%
  select(-trmA, -truB, -WT) %>%
  pivot_longer(c(-position, -gene, -AA, -anticodon, -number), names_to = c("treatment"), values_to = c("delta_mutation"))
...
plot1 <- ggplot(filter(temp,
  ...
  #extremely fancy coloring
  geom_tile(data=filter(temp, AA!="Und", delta_mutation >=0.1), fill="red4") +
  geom_tile(data=filter(temp, AA!="Und", delta_mutation <= -0.1), fill="blue4") +
  facet_grid(cols=vars(treatment)) +
  theme(strip.background = element_blank(),
        axis.title.y = element_blank())
...
ggplot(filter(temp,
              AA=="Val",
              number=="2"),
       aes(x=position, y=mutation, color=treatment, group=interaction(rep, treatment, gene))) +
  geom_line() +
  geom_text(aes(label=base, y=-0.03), color="black") +
  geom_vline(xintercept = 48, linetype=2, alpha=0.3)
...
ggplot(filter(temp,
  ...
  geom_line() +
  geom_text(aes(label=base, y=-0.03), color="black") +
  geom_vline(xintercept = 47, linetype=2, alpha=0.3)
...
ggplot(filter(temp,
              ...
              # anticodon=="GAA",
              number=="1"),
       aes(x=position, y=deletion/pileup, color=treatment, group=interaction(rep, treatment, gene))) +
  geom_line(alpha=0.3) +
  geom_text(aes(label=base, y=-0.07), color="black") +
  geom_point(data=filter(temp,
              AA %in% c("His","Tyr","Asp","Asn"),
              # anticodon=="GAA",
              number=="1",
             deletion/pileup >= 0.02,
             position %in% c(32,33,34,35,36))) +
...
write.csv(temp, file="./Single_KO_charging.csv", row.names = F)
...
write.csv(temp, file="./Single_KO_abundance.csv", row.names = F)
